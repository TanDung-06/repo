...
void removeAll(List*& L)
{
    ...
    L->p_tail = nullptr; //nguy hiểm lắm
}
...
void removeBefore(List*& L, int val)
{
    ...
    if (L->p_head != nullptr && L->p_head->p_next != nullptr)
    {
        ...
        {
            NODE* a = L->p_head->p_next;
            while (a->p_next != nullptr && a->p_next->key != val)
            {
                a = a->p_next;
            }
            if (a->p_next != nullptr)
...
void removeAfter(List*& L, int val)
{
    ...
    {
        NODE* a = L->p_head;
        while (a != nullptr && a->key != val)
        {
            a = a->p_next;
        }
        if (a != nullptr && a != L->p_tail)
        {
            if (a->p_next == L->p_tail)
            {
                removeTail(L);
            }
            else
...
{
    // Your code here //
    if (length(L) < pos or pos < 0 or L->p_head == nullptr)
    {
        return;
    }
    if (length(L) == pos)
...
bool addBefore(List*& L, int data, int val)
{
    ...
    while (b->p_next != nullptr && b->p_next->key != val)
    {
        b = b->p_next;
    }
    if (b->p_next != nullptr)
...
bool removeElement(List*& L, int key)
{
    ...
    while (a->p_next != nullptr && a->p_next->key != key)
    {
        a = a->p_next;
    }
    if (a->p_next != nullptr)
